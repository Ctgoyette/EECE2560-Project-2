...
class LinkList
{
    public:
        ...
        template <typename Y>
        friend ostream& operator<< (ostream& out, const LinkList<Y>&);
...
template <typename T>
void LinkList<T>::insert_after(T value)
{
    if (head == nullptr)
    {
        head = new Node<T>();
...
template <typename Y>
ostream& operator<< (ostream& out, const LinkList<Y>& list)
{
...
template <typename T>
T LinkList<T>::popNode(int index)
{
    curr = head;
